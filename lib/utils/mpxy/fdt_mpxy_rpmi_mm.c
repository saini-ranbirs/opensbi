...
{
	struct rpmi_mm_get_attributes_rsp rsp;
	...
	sbi_printf("OpenSBI %s: RPMI_MM_SRV_GET_ATTRIBUTES\n", __func__);
	...
	mmg->mm_version = rsp.mm_version;
	mmg->shmem_addr_lo = rsp.shmem_addr_lo;
	mmg->shmem_addr_hi = rsp.shmem_addr_hi;
	mmg->shmem_size = rsp.shmem_size;

#if __riscv_xlen == 32
	mm_region_addr = rsp.shmem_addr_lo;
#else
	mm_region_addr =
	    ((unsigned long)(rsp.shmem_addr_hi) << 32) | rsp.shmem_addr_lo;
#endif

	mm_region_size = rsp.shmem_size;
			     ...
			     struct mbox_xfer *xfer)
{
	...
	case RPMI_MM_SRV_COMMUNICATE:
		sbi_printf("OpenSBI %s: RPMI_MM_SRV_COMMUNICATE\n", __func__);
